...
{
    public class GrokAgent : IAiAgent
    {
        private readonly HttpClient _httpClient;
        private readonly string _modelName;
        private readonly string _apiKey;
        private readonly ILogger<GrokAgent> _logger;
        private readonly AsyncRetryPolicy _retryPolicy;

        private readonly ConcurrentDictionary<long, List<object>> _histories = new();
        private const int MaxHistoryMessages = 20;

        public GrokAgent(IConfiguration configuration, HttpClient httpClient, ILogger<GrokAgent> logger)
        {
            _apiKey = configuration["AiAgent:ApiKey"] ?? throw new ArgumentException("Grok API key is missing");
            _modelName = configuration["AiAgent:Model"] ?? "grok-beta";
            _httpClient = httpClient;
            _logger = logger;
            _httpClient.BaseAddress = new Uri(configuration["AiAgent:ApiUrl"] ?? "https://api.x.ai/v1/");

            // Инициализация политики ретраев
            _retryPolicy = AgentBot.Infrastructure.RetryPolicies.CreateDefaultRetryPolicy(logger, "Grok API");
        }
        ...
        public async Task<string> ProcessMessageAsync(long chatId, string message, List<IToolFunction> tools)
        {
            ...
            var grokTools = tools.Select(t => new
            {
                type = "function",
                function = new
                {
            ...
            while (iterations < 10)
            {
                var requestBody = new
                {
                    model = _modelName,
                    messages = history,
                    tools = grokTools.Any() ? grokTools : null,
                    tool_choice = grokTools.Any() ? "auto" : null
                };
                ...
                {
                    _logger.LogError("Grok API error: {Status} - {Response}", response.StatusCode, responseContent);
                ...
                if (toolCalls != null)
                {
                    foreach (var toolCall in toolCalls.Value.EnumerateArray())
                    {
                        ...
                        _logger.LogInformation("Grok Called function: {Name}", funcName);
